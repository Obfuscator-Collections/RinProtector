...
namespace KoiVM.Runtime.Execution.Internal {
	internal static class VMTrampoline {
		...
		static VMTrampoline() {
			foreach (var method in typeof(RinVM.RinVM).GetMethods(BindingFlags.Static | BindingFlags.NonPublic)) {
		...
		public static IntPtr CreateTrampoline(Module module, ulong codeAdr, uint key, VMFuncSig sig, uint sigId) {
			...
			lock (trampolines) {
				...
				if (ShouldBeTyped(sig))
					dm = CreateTrampolineTyped(RinInstance.GetModuleId(module), codeAdr, key, sig, sigId);
				else
					dm = CreateTrampolineNormal(RinInstance.GetModuleId(module), codeAdr, key, sig, sigId);
