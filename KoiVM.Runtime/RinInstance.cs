...
namespace RinVM {
	internal unsafe class RinInstance {
		[ThreadStatic] static Dictionary<Module, RinInstance> instances;
		static readonly object initLock = new object();
		static Dictionary<Module, int> initialized = new Dictionary<Module, int>();

		public static RinInstance Instance(Module module) {
			RinInstance inst;
			if (instances == null) {
				instances = new Dictionary<Module, RinInstance>();
			}
			if (!instances.TryGetValue(module, out inst)) {
				inst = new RinInstance(VMData.Instance(module));
		...
		public static RinInstance Instance(uint id) {
			int xd = Convert.ToInt32(id);
		...
		void Initialize() {
			...
			Ready(codeAddr, initFunc.EntryKey, initFunc.Signature, new object[0], null);
		}

		public object Initialize(uint id, object[] arguments, int value) {
			var export = Data.LookupExport(id);
			var codeAddr = (ulong)(Data.KoiSection + export.CodeOffset);
			return Ready(codeAddr, export.EntryKey, export.Signature, arguments,null);
		}

		public object Initialize(ulong codeAddr, int key, uint sigId, object[] arguments) {
			var lol = Convert.ToUInt32(key);
			var sig = Data.LookupExport(sigId).Signature;
			return Ready(codeAddr, lol, sig, arguments,null);
		}

		public void Initialize(uint id, void*[] typedRefs, void* retTypedRef) {
			var export = Data.LookupExport(id);
			var codeAddr = (ulong)(Data.KoiSection + export.CodeOffset);
			Run(codeAddr, export.EntryKey, export.Signature, typedRefs, retTypedRef);
		}

		public void Ready(ulong codeAddr, uint key, uint sigId, void*[] typedRefs, void* retTypedRef) {
			var sig = Data.LookupExport(sigId).Signature;
			Run(codeAddr, key, sig, typedRefs, retTypedRef);
		}

		object Ready(ulong codeAddr, uint key, VMFuncSig sig, object[] arguments, object empty) {
			if (currentCtx != null)
			...
			try {
				...
				VMDispatcher.Initialize(currentCtx);
